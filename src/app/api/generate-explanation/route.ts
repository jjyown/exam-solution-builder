...
export async function POST(request: Request) {
  try {
    ...
    const prompt = [
      ...
      "- [해설] 첫 단계에서 '이미지를 이렇게 읽었다'고 주어진 식을 한 줄로 요약해 적어(예: 주어진 식은 (√(2×∛4))^3 으로 해석한다). 이 해석이 틀리면 이후 전개는 모두 무효다.",
      "- 조합은 반드시 nCk 표기(예: 10C3)로 작성해.",
      "- [해설] 본문 첫 줄에 문제 번호(예: 17.)를 다시 쓰지 마.",
      "- [해설] 서술 스타일: 기본은 번호 없는 문단 풀이(먼저, 따라서, 이때, 한편 등). 1. 2. 3. 줄번호 매기기는 습관적으로 쓰지 말고, 분기·긴 절차가 필요할 때만 제한적으로 사용.",
      "- [정답], [해설] 형식을 엄격히 유지해.",
