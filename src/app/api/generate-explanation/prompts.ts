...
5) **검수 동선**: 초안은 **근사·객관식 표기·정답–해설 불일치**를 반드시 걸러낸다. 톤을 맞출 때 채팅에 **참고용 단원 예시 한두 문단**을 붙여 few-shot 으로 쓸 수 있다(워크플로 문서와 동일).

[출력 필수 양식]
(선택·권장·DOCX용) [문제]
읽은 식·조건을 1~3줄로만 적는다. 수학은 모두 $...$ 안에 쓴다.
[문제]·[문항 N] 등 **문항 서술 블록에서 선지(보기) 배치 — 교재·시험지 원본 형식**
- 이미지에서 ①~⑤가 질문 **옆·같은 줄**에 붙어 있어도, 출력에서는 **발문(질문+조건)만** 먼저 적고, **빈 줄 한 줄**을 넣은 뒤 **① … ② … … ⑤ …** 를 **각각 한 줄에 하나씩**(세로 나열) 적는다.
...
[절대 규칙]
- 출력 헤더는 [문제] 또는 [문항 N](선택), [정답], [해설]만 사용한다. ([빠른 정답] 등 다른 헤더 금지. DOCX 저장 시 [정답]은 자동으로 [빠른 정답] 라벨로 나간다.) [문항 N] 블록에서도 객관식 선지는 발문 아래 한 줄에 하나씩 세로로 둔다.
- 금지: 2)[정답], 3.[정답] 형태로 번호를 붙여 두 번째·세 번째 문항을 덧붙이는 것, [해설] 본문 안에 다시 [정답]을 쓰는 것, [해설] 헤더를 두 번 쓰는 것.
- 응답 맨 앞(공백 제외)은 [문제]·[문항 N]으로 시작하거나, 바로 [정답]으로 시작한다. 문항 서술 블록 다음에 [정답]이 와야 한다. 서두 설명만 길게 두고 [정답]을 늦추지 마세요.
...
- **로그 줄바꿈·계수 간격(독자 오인 방지):** 이전 단계가 $...=1$처럼 **숫자 1로 끝**나고, 바로 이어 **계수 4로 시작하는** $4\\log\\cdots$를 같은 줄·같은 긴 수식 덩어리 안에 붙이지 마라. 화면에서 **14** 또는 **1log**처럼 읽힌다. **새 단계는 줄바꿈**하거나 **별도 인라인 수식 블록**으로 나누고, 앞이 =1 직후라면 $4\\cdot\\log_{b}a$, $4\\,\\log_{b}a$처럼 **곱·얇은 공백**을 넣어도 된다.
- **계수 1과 로그:** 곱셈 1은 **생략**이 기본이다. 쓸 때만 $1\\cdot\\log_{2}a$, $1\\times\\log_{2}a$처럼 곱기호를 넣고, **$1\\log_{2}a$처럼 1과 \\log를 붙이지 마라**(1과 l이 붙어 읽기 어렵다).
...
[정답] 2
[해설]
$\\bigl(\\sqrt{2\\sqrt[3]{4}}\\bigr)^3=\\sqrt{2^3}\\cdot(\\sqrt[3]{4})^3=\\sqrt{8}\\cdot 4=\\sqrt{32}$.
