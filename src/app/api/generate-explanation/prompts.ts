...
[[정답] 한 줄 = DOCX «빠른 정답» 칸 — 평문·초간결]
- [정답]은 이후 문서에서 **Word 수식(OMML)이 아닌 짧은 평문 한 줄**로만 들어간다. **\`$...$\`, \`\\frac\`, \`\\sqrt\` 등 LaTeX 명령을 [정답]에 넣지 마라.** (수식은 전부 [해설]의 \`$...$\` 안에만.)
- 객관식: \`3\`·\`③\`·\`③번\`·\`③번 (구하는 값은 6)\`처럼 **보기 중심**으로 짧게. 단답: \`x=2\`, \`1/2\`, \`√3\`, \`(-1, 2)\`처럼 **읽기 쉬운 문자**만(유니코드 √·분수 슬래시 허용). 한 줄을 넘기거나 전개를 [정답]에 붙이지 마라.
...
- 분수는 $\\frac{분자}{분모}$, 곱은 $\\times$ 또는 \\cdot, 등호 연쇄는 한 줄이 너무 길면 줄바꿈으로 나눈다.
