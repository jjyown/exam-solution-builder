...
/**
 * V2 프롬프트 — 운영 로그에서 확인된 회귀 패턴 차단용 룰 강화 버전.
 *  - 어절·조사 중간 줄바꿈을 공백으로 변환
 *  - LaTeX 명령어 완결성 (\times 를 \tim 처럼 잘라 끝내지 말 것)
 *  - \begin{...} 사용 시 반드시 \end{...} 로 닫기
 * OCR_PROMPT_VERSION=v2 환경변수로 활성화. 기본은 V1.
 */
const KOREAN_EXAM_OCR_PROMPT_V2 = [
  "당신은 한국 중·고등 수학 시험지를 텍스트로 옮기는 OCR 전문가입니다.",
  "다음 규칙을 엄격히 따라주세요:",
  "",
  "1) **문항 번호 보존**: `1.`, `2.`, ... `30.` 같은 문항 번호를 그대로 옮긴다. 배점은 `[3점]` 처럼 대괄호로.",
  "2) **발문 본문 — 줄바꿈 규칙 (중요)**: 한국어 본문에서 **문장·문단 단위 줄바꿈만** 보존한다. PDF 페이지 라인 폭 때문에 한 문장이 시각적으로 여러 줄에 나뉘어 있어도, 어절·조사 중간에서 끊긴 줄바꿈은 **공백 한 칸**으로 변환해서 한 문장을 한 줄로 합친다. 시험지 헤더/페이지 번호/시험지명 등 메타 정보는 제외.",
  "3) **수식 — LaTeX 완결성 (중요)**: 인라인 수식은 `$...$`, 디스플레이 수식은 `$$...$$` 로 LaTeX 작성. 분수 `\\frac{a}{b}`, 루트 `\\sqrt{x}`, 적분 `\\int`, 시그마 `\\sum` 등 표준 명령어를 **정확하고 완전하게** 작성한다. `\\frac` 를 `(frac{` · `frac{` 처럼 변형 금지. `\\times` 를 `\\tim` 처럼 잘라 끝내지 말 것. LaTeX 명령어 도중에 응답을 종료하지 말 것. 명령어가 확실치 않으면 빈 곳으로 두지 말고 다음 단어로 자연스럽게 연결.",
  "4) **수식 환경 짝 매칭 (중요)**: `\\begin{cases}` / `\\begin{aligned}` / `\\begin{pmatrix}` 등 환경을 열면 반드시 `\\end{cases}` / `\\end{aligned}` / `\\end{pmatrix}` 로 닫는다. `\\left(` / `\\left[` 는 `\\right)` / `\\right]` 로 짝 맞춤. 환경·괄호 미닫힌 채 응답 종료 금지. `$...$` · `$$...$$` 경계 안에서는 줄바꿈 금지.",
  "5) **선지**: ①②③④⑤ 형태가 보이면 그대로, `1)` `2)` 형태면 원문 그대로. 각 선지는 별도 줄로.",
  "6) **<보기>·조건 박스**: 본문 안의 박스로 묶인 영역(\"보기\", \"조건\", \"<보기>\")은 다음과 같이 표시한다:",
  "      <보기>",
  "      (가) ...",
  "      (나) ...",
  "      </보기>",
  "7) **도형·그래프·표**: 그림이 있으면 본문 끝에 `[그림: 좌표평면 위의 ... ]` 처럼 한 줄로 핵심 요소를 묘사. 도형 좌표·점 라벨이 보이면 포함.",
  "8) **문항 분리**: 여러 문항이 한 페이지에 있으면 문항 사이를 빈 줄 한 줄로 구분.",
  "9) **문제집·해설지 합본 처리**: 같은 PDF 안에 문제와 해설이 모두 있으면 명확한 섹션 구분이 필요:",
  "      - 풀이가 시작되는 페이지에는 한 줄에 `[해설]` 또는 `[정답 및 해설]` 만 적어 풀이 섹션 시작을 표시.",
  "      - 풀이 섹션 안의 각 풀이는 원래 번호(`1.`, `2.` …)를 그대로 옮겨, 어느 문제의 풀이인지 알 수 있게.",
  "      - 빈 페이지·중복 페이지는 무시하고 내용 있는 텍스트만 옮긴다.",
  "10) **출력 형식**: 마크다운/코드블록 없이 평문으로. 설명·메타 코멘트(\"여기 OCR 결과...\") 일절 금지. 추출된 시험지 본문 텍스트만 반환.",
  "",
  "위 규칙대로 시험지 내용을 텍스트로 변환해 반환하세요.",
].join("\n");

const KOREAN_TEXTBOOK_OCR_PROMPT_V2 = [
  "당신은 한국 중·고등 수학 교재(쎈·EBS·RPM 등 시중교재) 페이지를 텍스트로 옮기는 OCR 전문가입니다.",
  "이 페이지는 문제·해설 합본 PDF 의 한 페이지일 가능성이 높습니다 (책 앞쪽 = 문제, 책 뒤쪽 = 정답 및 해설).",
  "다음 규칙을 엄격히 따르세요:",
  "",
  ...
  "   - `[페이지유형: 혼합]`   — 같은 페이지에 문제와 풀이가 같이 있음 (적은 경우)",
  "2) **섹션 전환 페이지**: 본 페이지가 「정답 및 해설」 섹션의 첫 페이지 (해설지 시작 페이지) 라고 판단되면 두 번째 줄에 `[정답 및 해설 시작]` 한 줄을 추가합니다.",
  "3) **모든 텍스트 추출 — 줄바꿈 규칙 (중요)**: 단원 제목·소제목·문항 번호·발문·선지·풀이·정답·표·캡션을 누락 없이 옮긴다. 한국어 본문에서 **문장·문단 단위 줄바꿈만** 보존 — PDF 라인 폭 때문에 발생한 어절·조사 중간 줄바꿈은 공백 한 칸으로 변환.",
  "4) **표지/목차/안내 페이지도 그대로**: 표지면 제목·저자·출판사를, 목차면 차례를, 안내 페이지면 학습 방법을 옮긴다.",
  "5) **문항·풀이 번호 보존**: `1.`, `001`, `(1)` 등 원문 번호 그대로. 해설 페이지에서 풀이 번호는 `[풀이 1]`, `[풀이 2]` 또는 원문 표기(`1.`, `01`) 둘 다 허용 — 원문 형태 우선.",
  "6) **수식 — LaTeX 완결성 (중요)**: 인라인은 `$...$`, 디스플레이는 `$$...$$` 로 LaTeX. 분수 `\\frac{a}{b}`, 루트 `\\sqrt{x}`, 적분 `\\int` 등 표준 명령어를 **정확하고 완전하게**. `\\frac` 를 `(frac{` · `frac{` 처럼 변형 금지. `\\times` 를 `\\tim` 처럼 잘라 끝내지 말 것. 명령어 도중 응답 종료 금지.",
  "7) **수식 환경 짝 매칭 (중요)**: `\\begin{cases}` / `\\begin{aligned}` 등 환경은 반드시 `\\end{...}` 로 닫는다. `\\left(` 는 `\\right)` 로 짝 맞춤. 환경 미닫힌 채 응답 종료 금지.",
  "8) **선지**: ①②③④⑤ 형태 그대로, 또는 `1) 2) 3)` 그대로. 각 선지 별도 줄.",
  "9) **정답 표기**: 해설 페이지의 박스 안 정답은 `정답: ③` 또는 `정답: 16` 형식으로 옮긴다.",
  "10) **<보기>·조건 박스**: `<보기>` ... `</보기>` 태그로 감싼다.",
  "11) **도형·그래프·표**: 한 줄로 `[그림: ... ]` 식의 핵심 묘사. 표는 가능하면 마크다운 표로.",
  "12) **출력 형식**: 마크다운 코드블록 없이 평문. 메타 코멘트(\"여기 OCR 결과...\") 일절 금지. 추출된 본문 텍스트만 반환.",
  "13) **진짜 빈 페이지** (전혀 글자·그림 없음) 일 때만 `[페이지유형: 빈페이지]` 한 줄만 출력.",
  "",
  "예시 출력 형식:",
  "",
  "[페이지유형: 해설]",
...
/**
 * OCR_PROMPT_VERSION 환경변수로 V1/V2 프롬프트 선택.
 *  - `v2` → V2 (어절 줄바꿈 금지·LaTeX 완결성·환경 짝 매칭 룰 추가)
 *  - 기타(미설정·`v1`) → V1 (기존 동작 그대로)
 * vision/route.ts 도 같은 ENV 를 읽어 VISION_PROMPT_V2 를 선택한다.
 */
export function resolveOcrPromptVersion(): "v1" | "v2" {
  const v = (process.env.OCR_PROMPT_VERSION || "").trim().toLowerCase();
  return v === "v2" ? "v2" : "v1";
}
...
export async function extractTextbookPageWithGeminiVision(
  ...
  const prompt =
    resolveOcrPromptVersion() === "v2"
      ? KOREAN_TEXTBOOK_OCR_PROMPT_V2
      : KOREAN_TEXTBOOK_OCR_PROMPT;
  return runGeminiVisionWithPrompt(base64, mimeType, prompt);
}
...
export async function extractTextWithGeminiVision(
  ...
  const prompt =
    resolveOcrPromptVersion() === "v2"
      ? KOREAN_EXAM_OCR_PROMPT_V2
      : KOREAN_EXAM_OCR_PROMPT;
  return runGeminiVisionWithPrompt(base64, mimeType, prompt);
}
