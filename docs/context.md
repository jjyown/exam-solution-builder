...
| 날짜 | 결정 | 이유 | 영향 범위 |
|------|------|------|-----------|
| 2026-05-06 | 5차 전역 병합(동일 번호·유사 본문) 직후, 정렬된 `LinePiece` 중 인접·동일 텍스트는 bbox 합집합으로 1개로 합친다 | 세그먼트 단위 union-find만으로는 “한 세그먼트 = 한 OCR 블록 한 줄(piece)” 구조에서 병합 후 `segment_join_body`가 동일 블록을 두 번 이어 붙이는 부작용이 생긴다. piece 단계 dedupe로 `page64`류 이중 파일의 본문 품질을 맞춘다. | `scripts/textbook_page_split_mathpix.py` |
| 2026-05-06 | 4차 보강(머리말 노이즈·인접 동일 owner·인접 중복 본문 제거)은 도입하되, `page64/page79` 고분할 해소는 다음 규칙으로 넘긴다 | `여러가지 순열` 재검증에서 strict 혼입 위반 0·`skipped=1`은 달성했지만, `page64=9`, `page79=7`은 유지되었다. 잔여는 동일 번호가 비슷한 본문으로 분리되는 케이스라 non-adjacent 포함 병합(번호+IoU+문장 유사도) 기준이 필요하다. | `scripts/textbook_page_split_mathpix.py`, `scratch/ocr-mapping-runs/tmp_split_v3_force/**` |
| 2026-05-06 | 운영 구분 고정: `교재 참고자료`는 실사용, `scratch/ocr-mapping-runs`는 테스트 전용으로 본다 | 분할 재실행 시 구/신 경로가 테스트 폴더에 공존하면 수량이 2배로 보이는 혼란이 발생한다. 운영 경로와 검증 경로를 문서/README로 명확히 분리해 혼용을 방지한다. | `교재 참고자료/README.md`, `scratch/ocr-mapping-runs/README.md`, `docs/TEXTBOOK_WORKFLOW.md` |
| 2026-05-06 | 분할 md 저장 전 `printedNumber`를 다단 fallback으로 추정해 `n/a`를 줄인다 | OCR 줄 타입/위치 오인식으로 세그먼트 owner가 비는 페이지가 존재해 감사에서 `skipped(no printedNumber)`가 누적됨. 출력 직전 `printed_number → [정답] 헤더 → 문항 시작 패턴` 순으로 보강하면 1:1 매핑 점검 가능 범위를 넓힐 수 있다. | `scripts/textbook_page_split_mathpix.py` |
| 2026-05-06 | 기존 `교재 참고자료`에 남은 `*.pdf` 유형 폴더·frontmatter는 `normalize_textbook_reference_layout.py`로 stem 정리한다 | 태그 매칭(`textbookReferenceSelector`)이 `type` 문자열에 의존하므로 `9. 단원.pdf` vs `9. 단원` 불일치를 제거한다. 감사는 `audit_textbook_reference_tree.py`로 주기적으로 수행한다. | `tools/normalize_textbook_reference_layout.py`, `tools/audit_textbook_reference_tree.py`, `docs/TEXTBOOK_WORKFLOW.md` |
| 2026-05-06 | 대용량 OCR·분할 실험 산출은 `scratch/ocr-mapping-runs/`에만 두고, 정식 참고자료는 `교재 참고자료/<단원>/<유형>/<난이도>/`로 구분한다 | 루트 `tmp_*` 다발은 “방치”로 오인되기 쉽고, git 추적·검색 노이즈가 커진다. 입력 메타는 **파일명을 경로 깊이에서 제외**해 `유형=파일.pdf` 같은 오분류를 막는다. | `scratch/ocr-mapping-runs/README.md`, `scripts/build-textbook-reference.mts`, `.gitignore`, `docs/TEXTBOOK_WORKFLOW.md` |
| 2026-05-06 | Cursor 규칙에 레포 구조(A 시험 해설 / B 교재 분할·참고)와 “problemNN ≠ API 자동 이미지 매핑” 경계를 명문화한다 | 채팅이 끊겨도 에이전트가 분할 스크립트와 `generate-explanation`/`textbookReferenceSelector`를 혼동하지 않도록 한다. | `.cursor/rules/codebase-architecture.mdc`, `.cursorrules` |
| 2026-05-06 | 원격 배포(Railway)는 저장소에 추적된 파일만 빌드하므로, 로컬에만 있던 reasoning·파이프라인 보조 모듈은 반드시 커밋에 포함한다 | 빌드 로그에서 `@/lib/reasoning/explanationFormatPolicy` 등 Module not found가 발생한 원인이 미추적 파일이었음. `.env.local`은 원격에 올리지 않고 플랫폼 환경변수로만 주입한다. | CI/Railway 배포, `src/lib/reasoning/*`, `src/lib/quality/*`, `src/lib/assembly/*`, `src/lib/recognition/questionVisuals.ts` |
| 2026-05-06 | `textbook:build-reference` 입력은 `교재 입력`뿐 아니라 `참고용 문제` 루트(또는 단원 하위 폴더)를 그대로 `--input`으로 쓸 수 있다 | 레포에 PDF 예외 추적이 `참고용 문제/**/*.pdf`로 잡혀 있어, 별도 복사 없이 동일 스크립트로 분할·OCR 검증을 반복하기 쉽다. | `docs/TEXTBOOK_WORKFLOW.md`, `scripts/build-textbook-reference.mts` |
| 2026-05-06 | 교재 분할 산출 md는 `tools/audit_textbook_split_md.py`로 `printedNumber`·`[정답]` 헤더 정합을 재현 가능하게 집계한다 | PDF·Mathpix 없이도 이전 산출(`tmp_split_output_*`) 대비 3차 적용 효과를 숫자로 비교할 수 있게 한다. `--strict --max-answer-headers`로 빠른정답 목록 페이지를 대략 배제할 수 있다. | `tools/audit_textbook_split_md.py`, `docs/TEXTBOOK_WORKFLOW.md` |
| 2026-05-06 | 교재 bbox 분할 3차로 세그먼트 내부의 타 문항 `[정답]` 헤더에서 강제 분할한다 | 실데이터에서 `printedNumber=8` 세그먼트에 `7) [정답]`이 섞여 `matchKey`와 본문이 어긋날 수 있음. 줄 단위 `LinePiece`로 bbox를 보존한 뒤, 소유 번호와 다른 정답 헤더에서만 쪼개 오탐을 줄인다. | `scripts/textbook_page_split_mathpix.py` |
| 2026-05-06 | Multi-Mapping 과분할 억제 1차로 문항 시작 판정에 `line type + 좌표 + 번호 진행성` 필터를 결합하고 세그먼트 상한(>20) 시 단일 폴백을 적용한다 | 실데이터에서 극단 과분할(63/71)이 확인되어, 번호 패턴 단독 매칭으로는 안정 운용이 어려움. 필터 결합 후 동일 PDF 재검증에서 총 산출량이 `403 -> 159`로 감소하며 극단치가 완화됨. | `scripts/textbook_page_split_mathpix.py`, `tmp_split_output_tuned/**` |
| 2026-05-06 | `확률과 통계/1. 여러가지 순열.pdf` 실데이터에서 Multi-Mapping 산출을 검증한 결과, 문항 분할은 동작하지만 일부 페이지에서 번호 패턴 오인식으로 과분할이 발생할 수 있다 | 페이지별 `2~4문항`이 정상인 구간이 다수였으나 후반 고난도 페이지에서 `63/71`처럼 비정상 과분할이 관찰됨. 다음 튜닝은 문항 시작 패턴을 상단 좌측/줄 타입/최소 간격 조건으로 제한해야 한다. | `scripts/textbook_page_split_mathpix.py`, `tmp_split_output/**` |
| 2026-05-06 | 현재 교재 참고자료 md 기반 `textbookReferenceSelector` 프롬프트 주입 스모크 테스트 | 기존 bbox split/문항 분할 입력이 없어도, unit/type/difficulty 태그 기준 선택 및 프롬프트 블록 생성이 정상 동작함을 확인(멀티매핑 bbox 검증은 원본 PDF/페이지 PNG 입력이 필요) | `scripts/textbook_reference_selector_smoke.mts`, `src/lib/reasoning/textbookReferenceSelector.ts` |
| 2026-05-06 | 다문항 교재 페이지는 Mathpix `line_data.cnt`로 문항 구간을 나눈 뒤 Pillow로 크롭해 `*_problemNN.png`+`md` 1:1 세트를 만든다 | 페이지 OCR 1건=md 1건 가정은 실제 교재(한 페이지 3~4문항)와 충돌하므로, bbox 기반 물리 분할이 해설·DOCX 입력 품질에 필수에 가깝다. 또한 `scripts/build-textbook-reference.mts`의 PDF 분기에 해당 split 로직을 자동 연동해 운영 비용/누락을 줄인다. | `scripts/textbook_page_split_mathpix.py`, `scripts/build-textbook-reference.mts`, `docs/TEXTBOOK_WORKFLOW.md` |
